...
describe('POST /timers/start', () => {
  ...
  it('200 - 타이머 생성 후 형식에 맞게 응답한다.', async () => {
    const res = await request(app)
      ...
      .send({ goalId });
    console.log(`/timers/start goalId: ${goalId}, userId: ${userId}`);
  ...
  // 해당 사용자의 goalId가 존재하지 않음
  it('404 - 해당 사용자의 goalId가 아니면 목표 없음 에러를 반환한다', async () => {
    ...
    const otherCategory = await prisma.category.create({
      data: {
        userId: otherUser.id,
        name: `${TEST_PREFIX}_OTHER_CATEGORY`,
        unit: '페이지',
      },
    });
  ...
  it('409 - 이미 실행 중인 타이머가 있으면 충돌을 반환한다', async () => {
    const now = new Date();
