...
describe('Goal API Integration', () => {
  ...
  beforeEach(async () => {
    ...
    goalLogId1 = goalLog1.id;
    goalLogId2 = goalLog2.id;
  ...
  afterEach(async () => {
    ...
    await prisma.timerLog.deleteMany({
      where: {
        OR: [{ goalId: { in: testGoalIds } }, { userId: { in: testUserIds } }],
      },
    });
  ...
  describe('GET /goals', () => {
    ...
    describe('401 - UNAUTHORIZED', () => {
      it('유효하지 않은 토큰이면 인증 에러를 반환한다', async () => {
        const res = await request(app)
          .get('/goals')
    ...
    describe('500 - INTERNAL_SERVER_ERROR', () => {
      it('서버 내부 예외가 발생하면 에러를 반환한다', async () => {
        jest
          .spyOn(goalService, 'getGoalListService')
  ...
  describe('POST /goals', () => {
    ...
    describe('400 - BAD_REQUEST', () => {
      it('시작일이 종료일보다 늦으면 에러를 반환한다', async () => {
        const res = await request(app)
          .post('/goals')
          .set('Cookie', [`token=${authToken}`])
          .send({
    ...
    describe('404 - CATEGORY_NOT_FOUND', () => {
      it('존재하지 않는 카테고리면 에러를 반환한다', async () => {
        const res = await request(app)
          .post('/goals')
          .set('Cookie', [`token=${authToken}`])
          .send({
    ...
    // 실제 service import 후 사용
    describe('500 - INTERNAL_SERVER_ERROR', () => {
      it('서버 내부 예외가 발생하면 에러를 반환한다', async () => {
        jest
          .spyOn(goalService, 'createGoalService')
          .mockRejectedValueOnce(new Error('DB error'));
  ...
  describe('GET /goals/:goalId/detail', () => {
    describe('200 - OK', () => {
      it('dailyProgress 각 객체에 goalLogId를 포함해 상세 정보를 조회한다', async () => {
        const res = await request(app)
          .get(
            `/goals/${goalId}/detail?startDate=2026-03-14&endDate=2026-03-15`,
          )
          .set('Cookie', [`token=${authToken}`]);
        ...
        expect(res.body.data.dailyProgress).toEqual(
          expect.arrayContaining([
            expect.objectContaining({
              goalLogId: goalLogId1,
              date: '2026-03-14',
            }),
            expect.objectContaining({
              goalLogId: goalLogId2,
              date: '2026-03-15',
            }),
          ]),
        );
      ...
      it('목표 기간과 겹치지 않는 조회 범위면 dailyProgress를 빈 배열로 반환한다', async () => {
        const res = await request(app)
          .get(
            `/goals/${goalId}/detail?startDate=2025-01-01&endDate=2025-01-02`,
          )
          .set('Cookie', [`token=${authToken}`]);
    ...
    describe('400 - BAD_REQUEST', () => {
      it('잘못된 날짜 범위면 에러를 반환한다', async () => {
        const res = await request(app)
          .get(
            `/goals/${goalId}/detail?startDate=2026-03-20&endDate=2026-03-10`,
          )
          .set('Cookie', [`token=${authToken}`]);
    ...
    describe('404 - GOAL_NOT_FOUND', () => {
      it('존재하지 않는 목표면 에러를 반환한다', async () => {
        const nonExistentGoalId = goalId + 9999;

        const res = await request(app)
          .get(`/goals/${nonExistentGoalId}/detail`)
          .set('Cookie', [`token=${authToken}`]);
    ...
    describe('500 - INTERNAL_SERVER_ERROR', () => {
      it('서버 내부 예외가 발생할 경우 반환한다', async () => {
        jest
          .spyOn(goalService, 'getGoalDetailService')
          .mockRejectedValueOnce(new Error('DB error'));
  ...
  describe('PATCH /goals/:goalId', () => {
    describe('200 - OK', () => {
      it('목표 수정 시 quota를 재계산하고 dailyProgress에 goalLogId를 포함해 반환한다', async () => {
        const res = await request(app)
          .patch(`/goals/${goalId}`)
          .set('Cookie', [`token=${authToken}`])
          .send({
        ...
        expect(res.body.data.period.endDate).toBe('2026-03-20');

        expect(res.body.data).toHaveProperty('dailyProgress');
        expect(Array.isArray(res.body.data.dailyProgress)).toBe(true);

        if (res.body.data.dailyProgress.length > 0) {
          expect(res.body.data.dailyProgress[0]).toHaveProperty('goalLogId');
        }
      });
    });

    describe('400 - BAD_REQUEST', () => {
      it('수정할 값이 없으면 에러를 반환한다', async () => {
        const res = await request(app)
          .patch(`/goals/${goalId}`)
          .set('Cookie', [`token=${authToken}`])
          .send({});
      ...
      it('totalAmount가 1 이상의 정수가 아니면 에러를 반환한다', async () => {
        const res = await request(app)
          .patch(`/goals/${goalId}`)
          .set('Cookie', [`token=${authToken}`])
          .send({
      ...
      it('endDate 형식이 잘못되면 에러를 반환한다', async () => {
        const res = await request(app)
          .patch(`/goals/${goalId}`)
          .set('Cookie', [`token=${authToken}`])
          .send({
      ...
      it('종료일이 시작일보다 빠르면 에러를 반환한다', async () => {
        const res = await request(app)
          .patch(`/goals/${goalId}`)
          .set('Cookie', [`token=${authToken}`])
          .send({
    ...
    describe('500 - INTERNAL_SERVER_ERROR', () => {
      it('서버 내부 예외가 발생할 경우 반환한다', async () => {
        jest
          .spyOn(goalService, 'updateGoalService')
          .mockRejectedValueOnce(new Error('DB error'));
  ...
  describe('DELETE /goals/:goalId', () => {
    ...
    describe('404 - GOAL_NOT_FOUND', () => {
      it('존재하지 않는 목표를 삭제하려 하면 에러를 반환한다', async () => {
        const nonExistentGoalId = goalId + 9999;
    ...
    describe('500 - INTERNAL_SERVER_ERROR', () => {
      it('서버 내부 예외가 발생할 경우 반환한다', async () => {
        jest
          .spyOn(goalService, 'deleteGoalService')
          .mockRejectedValueOnce(new Error('DB error'));
  ...
  describe('GET /goals/today', () => {
    describe('200 - OK', () => {
      it('오늘 목표 리스트 각 객체에 goalLogId를 포함해 반환한다', async () => {
        ...
        const todayGoalLog = await prisma.goalLog.create({
          data: {
        ...
        expect(todayGoal).toHaveProperty('goalLogId');
        expect(todayGoal.goalLogId).toBe(todayGoalLog.id);
      });

      it('오늘 진행 중인 목표가 없으면 빈 배열을 반환한다', async () => {
      ...
      it('실행 중인 타이머가 있으면 isTimerRunning이 true다', async () => {
        ...
        expect(targetGoal.goalLogId).toBe(runningGoalLog.id);
  ...
  describe('GET /goals/today/complete', () => {
    describe('200 - OK', () => {
      ...
      it('오늘 진행 중인 목표가 없으면 0을 반환한다', async () => {
        const emptyUser = await prisma.user.create({
          data: {
    ...
    describe('500 - INTERNAL_SERVER_ERROR', () => {
      it('서버 내부 예외가 발생할 경우 반환한다', async () => {
        jest
          .spyOn(goalService, 'getTodayGoalCompletionService')
          .mockRejectedValueOnce(new Error('DB error'));
